...
function buildUserMessage(bookTitle: string, author: string, review: string, description: string): string {
    return `## 책 정보
- 제목: ${bookTitle}
- 저자: ${author}
- 책 소개: ${description}
...
async function analyzeWithGemini(
    ...
    const userMessage = buildUserMessage(bookTitle, author, review, description);
...
async function analyzeWithOpenAI(
    ...
    const response = await withRetry(async () => {
        ...
        if (!res.ok) {
            ...
            throw new Error(`OpenAI API 오류: ${res.status} ${errorData?.error?.message || res.statusText}`);
        }
...
function parseResponse(text: string): ReviewAnalysis {
    try {
...
/**
 * 설정에 따라 Gemini 또는 OpenAI로 분석을 수행합니다.
 */
export async function analyzeReview(
    ...
    if (settings.aiProvider === 'openai') {
        ...
        return analyzeWithOpenAI(settings.openaiApiKey, settings.openaiModel, bookTitle, author, review, description);
    } else {
        if (!settings.geminiApiKey) throw new Error('Gemini API 키가 설정되지 않았습니다.');
        return analyzeWithGemini(settings.geminiApiKey, bookTitle, author, review, description);
    }
}
