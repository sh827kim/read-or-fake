...
import type { WebSettings } from './web-storage';
...
async function analyzeWithOpenAI(
    ...
    const response = await withRetry(async () => {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                `OpenAI API 오류: ${res.status} ${errorData?.error?.message || res.statusText}`,
            );
        }
...
/**
 * 설정에 따라 Gemini 또는 OpenAI로 감상문 분석을 수행합니다.
 * API 키는 클라이언트 브라우저에서 직접 사용되며, 서버로 전송되지 않습니다.
 */
